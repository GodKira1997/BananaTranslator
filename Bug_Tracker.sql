...
Create table Bug(bug_id int not null GENERATED ALWAYS AS IDENTITY , title varchar(30) not null,description varchar(200) not null,prj_id int ,open_date date not null, marked_for_closing varchar(5) ,closed_on date ,status varchar(15) not null,severity varchar(15) not null,FOREIGN KEY(prj_id) references Project(prj_id),primary key(bug_id,prj_id));

Create table Bug_user(bug_id  int not null,user_id int not null,prj_id int not null,primary key(bug_id,prj_id,user_id),FOREIGN KEY(bug_id,prj_id) references Bug(bug_id,prj_id),FOREIGN KEY(user_id) references Users(user_id));
